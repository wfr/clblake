// Blake-256 reference implementation
...
void blake256_ref_final( state256 *S, uint8_t *out )
{
  ...
  else
  {
    if ( S->buflen < 55 )   /* enough space to fill the block  */
    {
      ...
      blake256_ref_update( S, padding, 55 - S->buflen );
    }
    else   /* need 2 compressions */
    {
      S->t[0] -= 512 - ( S->buflen << 3 );
      blake256_ref_update( S, padding, 64 - S->buflen );
      S->t[0] -= 440;
      blake256_ref_update( S, padding + 1, 55 );
      S->nullt = 1;
    }

    blake256_ref_update( S, &zo, 1 );
    S->t[0] -= 8;
  }

  S->t[0] -= 64;
  blake256_ref_update( S, msglen, 8 );
...
void blake256_ref_hash( uint8_t *out, const uint8_t *in, uint64_t inlen )
{
  state256 S;
  blake256_ref_init( &S );
  blake256_ref_update( &S, in, inlen );
  blake256_ref_final( &S, out );
}
